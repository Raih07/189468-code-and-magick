...
window.renderStatistics = function (ctx, names, times) {
  ...
  for (var i = 0; i < names.length; i++) {
    ...
    ctx.fillRect(BAR_X_START + (BAR_INDENT + BAR_WIDTH) * i, BAR_Y_START + BAR_HEIGHT - (Math.round(times[i]) * BAR_HEIGHT) / maxTime + FONT_HEIGHT / 2, BAR_WIDTH, (Math.round(times[i]) * BAR_HEIGHT) / maxTime);

    ctx.fillStyle = '#000000';
    ctx.fillText(Math.round(times[i]), BAR_X_START + (BAR_INDENT + BAR_WIDTH) * i, BAR_Y_START + BAR_HEIGHT - (Math.round(times[i]) * BAR_HEIGHT) / maxTime);
